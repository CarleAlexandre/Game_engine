...
void	gen_render_chunk(world_t *world, Vector3 player_pos) {
	...
	world->render_chunk[0] = svo_get_node(&(float){cx, cy, cz}, world->chunk);

	world->render_chunk[1] = svo_get_node(&(float){cx, cy + 1, cz}, world->chunk);

	world->render_chunk[2] = svo_get_node(&(float){cx, cy - 1, cz}, world->chunk);

	world->render_chunk[3] = svo_get_node(&(float){cx + 1, cy, cz}, world->chunk);
	
	world->render_chunk[4] = svo_get_node(&(float){cx - 1, cy, cz}, world->chunk);
	
	world->render_chunk[5] = svo_get_node(&(float){cx, cy, cz + 1}, world->chunk);
	
	world->render_chunk[6] = svo_get_node(&(float){cx, cy, cz - 1}, world->chunk);

	world->render_chunk[7] = svo_get_node(&(float){cx, cy + 1, cz + 1}, world->chunk);
	world->render_chunk[8] = svo_get_node(&(float){cx, cy + 1, cz - 1}, world->chunk);
	world->render_chunk[9] = svo_get_node(&(float){cx + 1, cy + 1, cz}, world->chunk);
	world->render_chunk[10] = svo_get_node(&(float){cx - 1, cy + 1, cz}, world->chunk);

	world->render_chunk[11] = svo_get_node(&(float){cx + 1, cy + 1, cz + 1}, world->chunk);
	world->render_chunk[12] = svo_get_node(&(float){cx + 1, cy + 1, cz - 1}, world->chunk);
	world->render_chunk[13] = svo_get_node(&(float){cx - 1, cy + 1, cz + 1}, world->chunk);
	world->render_chunk[14] = svo_get_node(&(float){cx - 1, cy + 1, cz - 1}, world->chunk);

	world->render_chunk[15] = svo_get_node(&(float){cx, cy - 1, cz + 1}, world->chunk);
	world->render_chunk[16] = svo_get_node(&(float){cx, cy - 1, cz - 1}, world->chunk);
	world->render_chunk[17] = svo_get_node(&(float){cx + 1, cy - 1, cz}, world->chunk);
	world->render_chunk[18] = svo_get_node(&(float){cx - 1, cy - 1, cz}, world->chunk);

	world->render_chunk[19] = svo_get_node(&(float){cx + 1, cy - 1, cz + 1}, world->chunk);
	world->render_chunk[20] = svo_get_node(&(float){cx + 1, cy - 1, cz - 1}, world->chunk);
	world->render_chunk[21] = svo_get_node(&(float){cx - 1, cy - 1, cz + 1}, world->chunk);
	world->render_chunk[22] = svo_get_node(&(float){cx - 1, cy - 1, cz - 1}, world->chunk);

	world->render_chunk[23] = svo_get_node(&(float){cx + 1, cy, cz + 1}, world->chunk);
	world->render_chunk[24] = svo_get_node(&(float){cx - 1, cy, cz + 1}, world->chunk);
	world->render_chunk[25] = svo_get_node(&(float){cx + 1, cy, cz - 1}, world->chunk);
	world->render_chunk[26] = svo_get_node(&(float){cx + 1, cy, cz - 1}, world->chunk);
